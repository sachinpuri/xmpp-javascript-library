...
//                        if($(xml).find("iq > query > item").attr("ask") !== undefined){//Presence Request
//                            
//                        }else{//Roster Entry
    ...
    parseMessages:function(xml){
        if($(xml).find("sent").length>0 && $(xml).find("sent").attr("xmlns") === "urn:xmpp:carbons:2"){ //Carbon Messages
            ...
            if($(xml).find("delay").length>0){
                var date = $(xml).find("delay").attr("stamp").substr(0,10).split('-');
                var time = $(xml).find("delay").attr("stamp").substr(11,8).split(':');
                ...
                objMessage['delay']['time'] = dateTime.toLocaleString().replace(new RegExp("/",'g'),"-");
            }
